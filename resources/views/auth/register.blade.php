...
<section class="login">
  <form method="POST" class="mt-8" action="{{ route('register') }}">
      ...
      <div class="button-group">
          ...
          <input aria-label="Phone number"  autofocus name="mobile" type="text" value="{{ old('mobile') }}" required autocomplete="mobile"  placeholder="ユーザーネームを入力">
      </div>
      <div class="button-group">
          <h2 class="jap-stroke text-center" data-text="電話番号">電話番号</h2>
          <input aria-label="username" name="name" type="text" required  placeholder="電話番号を入力" >
      </div>
      <button type="submit">
